...
\justify HaDeX provides additional tools for decision making. A perfect example is quality_control() function that shows how the mean uncertainty per peptide changes with "out time" parameter change. The user can see if chosen out time is sufficient for acceptable uncertainty values, or is it better to choose different value.
